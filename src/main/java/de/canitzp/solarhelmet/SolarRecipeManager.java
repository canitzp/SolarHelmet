...
public class SolarRecipeManager {
    ...
    public static Recipe<?> creationRecipe(final Item helmet, final ResourceLocation craftingId){
        ...
        return new UpgradeRecipe(craftingId, Ingredient.of(helmet), Ingredient.of(SolarHelmet.SOLAR_MODULE_ITEM.get()), outputStack){
            @Override
            public @NotNull ItemStack assemble(@NotNull Container container) {
                ItemStack assembled = super.assemble(container);
